...
export default {
  ...
  methods: {
    ...
    buscar() {
      ...
      if (query) {
        this.searchGet = true
        this.searchValues = this.eventos.filter(ev => 
          ev.evento_titulo?.toUpperCase().includes(query) ||
          ev.evento_descripcion?.toUpperCase().includes(query) ||
          ev.evento_lugar?.toUpperCase().includes(query)
        )
